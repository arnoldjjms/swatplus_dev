      ...
      case ("stor")
        ...
        if (wtow_om_stor(j)%flo > wtow_min) then
          ...
          poud_om(ipou)%pod(ipod) = rto * wtow_om_stor(j)
      ...
      case ("can")
        ...
        if (canal_om_stor(j)%flo >= can_min) then
          ...
          poud_om(ipou)%pod(ipod) = rto * canal_om_stor(j)
      ...
      case ("res")
        ...
        if (res(j)%flo > res_min) then
          ...
          poud_om(ipou)%pod(ipod) = rto * res(j)
          res(j) = (1. - rto) * res(j)
        end if
      ...
      case ("aqu") 
        ...
        if (aqu_d(j)%dep_wt < pou(ipou)%pod(ipod)%const_min) then
          ...
          poud_om(ipou)%pod(ipod)%flo = rto * aqu_d(j)%stor
          poud_om(ipou)%pod(ipod)%no3 = rto * aqu_d(j)%no3_st
          poud_om(ipou)%pod(ipod)%solp = rto * aqu_d(j)%minp
        end if
